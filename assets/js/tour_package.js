...
if (url_name[0] == main_url && !url_name[1]) {
  ...
  if (moreLang) {
    const allLang = document.querySelector(".all_lang") ?? null;
    if (allLang) {
      moreLang.addEventListener("mouseenter", () => allLang.classList.remove("hidden"));
      moreLang.addEventListener("mouseleave", () => allLang.classList.add("hidden"));
    }
  }
