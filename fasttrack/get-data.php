...
function enx_create_list($items)
{
    foreach ($items as $item) {
        ?>
        <div data-x-data data-x-ref="losAngeles" data-x-intersect="anime({ targets: $refs.losAngeles, translateY: [100, 0], opacity: [0, 1], duration: 500 ,easing: 'easeOutQuad' })" style="flex: 1 1 0">
            <a href="<?php echo "/" . AIRPORT_SERVICE_LINK . "/" . $item->slug ?>" class="group flex-1 block relative bg-cover rounded-2xl xl:my-0 overflow-hidden w-full" style="min-height: 480px">
                <div class="absolute bg-cover bg-center origin-top w-full rounded-2xl transition duration-500 transform translate-y-[-10px] group-hover:scale-110" style="background-image: url(<?php echo $item->image_url ?>); height: 80%">
                </div>
                ...
                <div class="absolute bottom-10 w-full bg-secondary transition duration-500 group-hover:bg-primary py-5 px-4 rounded-2xl">
